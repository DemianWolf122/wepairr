...
const SvgBuilding = () => <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" strokeWidth="2" fill="none"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"></rect><path d="M9 22v-4h6v4"></path><path d="M8 6h.01"></path><path d="M16 6h.01"></path><path d="M12 6h.01"></path></path></svg>;
...
const SvgShare = () => <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" strokeWidth="2" fill="none"><circle cx="18" cy="5" r="3"></circle><circle cx="6" cy="12" r="3"></circle><circle cx="18" cy="19" r="3"></circle><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"></line><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"></line></svg>;
const SvgInstagram = () => <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>;
...
const SvgUpload = () => <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" strokeWidth="2" fill="none"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>;
...
const ACCENT_COLORS = [{ hex: '#2563eb' }, { hex: '#10b981' }, { hex: '#8b5cf6' }, { hex: '#e11d48' }, { hex: '#f97316' }, { hex: '#334155' }, { hex: '#ec4899' }, { hex: '#06b6d4' }, { hex: '#eab308' }, { hex: '#4f46e5' }, { hex: '#84cc16' }, { hex: '#b91c1c' }];
const TEXT_COLORS = [{ hex: '#000000' }, { hex: '#0f172a' }, { hex: '#334155' }, { hex: '#1e3a8a' }, { hex: '#450a0a' }, { hex: '#ffffff' }, { hex: '#f8fafc' }, { hex: '#cbd5e1' }, { hex: '#e0f2fe' }, { hex: '#fef3c7' }, { hex: '#fffbeb' }, { hex: '#64748b' }];
const FONTS = [{ label: 'Inter', value: '"Inter", sans-serif' }, { label: 'Helvetica', value: '"Helvetica Neue", sans-serif' }, { label: 'Montserrat', value: '"Montserrat", sans-serif' }, { label: 'Poppins', value: '"Poppins", sans-serif' }];
...
function Settings({ config, onUpdate }) {
    ...
    // Subida de imagen local convertida a base64 para vista previa
    ...
    const shopStyles = useMemo(() => {
        ...
        if (getContrastRatio(accent, isDarkMode ? '#090e17' : '#ffffff') < 3.0) {
        ...
        const heroOverlayStyle = hasBanner
            ...
            : isDarkMode ? 'linear-gradient(to bottom, rgba(0,0,0,0.2), var(--shop-bg))' : 'linear-gradient(to bottom, rgba(255,255,255,0.7), var(--shop-bg))';
        ...
        <div className="settings-editor-layout">
            ...
            <div className="settings-controls-panel glass-effect">
                ...
                <div className="accordions-container">
                    ...
                    {/* IDENTIDAD CON LÍMITES DE CARACTERES */}
                            ...
                            <div className="accordion-content">
                                <div className="settings-form-group">
                                    <label className="settings-label">
                                        <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Nombre Visible:</span><span className="char-count">{config.nombreNegocio?.length || 0}/40</span></div>
                                        <input type="text" name="nombreNegocio" value={config.nombreNegocio || ''} onChange={handleChange} maxLength={40} className="settings-input" />
                                    </label>
                                    <label className="settings-label">
                                        <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Título Principal:</span><span className="char-count">{config.titulo?.length || 0}/60</span></div>
                                        <input type="text" name="titulo" value={config.titulo || ''} onChange={handleChange} maxLength={60} className="settings-input" />
                                    </label>
                                    <label className="settings-label">
                                        <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Descripción Corta:</span><span className="char-count">{config.descripcion?.length || 0}/160</span></div>
                                        <textarea name="descripcion" value={config.descripcion || ''} onChange={handleChange} maxLength={160} className="settings-input settings-textarea" />
                                    </label>
                                </div>
                            </div>
                        )}
                        ...
                        {seccionAbierta === 'apariencia' && (
                            <div className="accordion-content">
                                <div className="toggle-box glass-input-effect" style={{ marginBottom: '20px' }}>
                                    <div><strong className="toggle-title">Modo Oscuro</strong><span className="toggle-desc">Activa el fondo oscuro.</span></div>
                                ...
                                <PremiumGate isPremium={isPremium}>
                                    ...
                                    <div className="color-presets-grid text-grid" style={{ marginBottom: '25px' }}>
                                        {TEXT_COLORS.map(preset => {
                                            const isSafe = checkColorSafety(preset.hex, currentBgColor);
                                            return <button key={'sub-' + preset.hex} type="button" className={`color-preset-btn text-color-btn ${config.colorSubtitulo === preset.hex ? 'active' : ''} ${!isSafe ? 'disabled-contrast' : ''}`} style={{ backgroundColor: preset.hex }} onClick={() => isSafe && onUpdate({ ...config, colorSubtitulo: preset.hex })} />;
                                        })}
                                    </div>
                                    <div className="settings-form-group">
                    ...
                    <div className={`accordion-item ${seccionAbierta === 'multimedia' ? 'active' : ''}`}>
                        ...
                        {seccionAbierta === 'multimedia' && (
                            <div className="accordion-content">
                                <PremiumGate isPremium={isPremium}>
                                    <div className="settings-form-group">
                                        <label className="settings-label">Imagen de Portada (Banner):
                                            {/* Subida de archivo Real simulada */}
                                            <div className="file-upload-wrapper">
                                                <input type="file" accept="image/*" onChange={handleImageUpload} id="banner-upload" className="file-upload-hidden" />
                                                <label htmlFor="banner-upload" className="btn-upload"><SvgUpload /> Subir desde PC</label>
                                                <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', display: 'block', marginTop: '5px' }}>O pega una URL directa:</span>
                                            </div>
                                            <input type="text" name="bannerUrl" value={config.bannerUrl || ''} onChange={handleChange} className="settings-input" placeholder="https://..." />
                                        </label>
                    ...
                    <div className={`accordion-item ${seccionAbierta === 'redes' ? 'active' : ''}`}>
                        ...
                        {seccionAbierta === 'redes' && (
                            <div className="accordion-content">
                                <div className="settings-form-group">
                                    <label className="settings-label">
                                        <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>WhatsApp (Número):</span><span className="char-count">{config.whatsapp?.length || 0}/15</span></div>
                                        <input type="text" name="whatsapp" value={config.whatsapp || ''} onChange={handleChange} maxLength={15} className="settings-input" placeholder="Ej. 1123456789" />
                                    </label>
                    ...
                    <div className={`accordion-item ${seccionAbierta === 'funcionalidades' ? 'active' : ''}`}>
                        <div className="accordion-header" onClick={() => toggleSeccion('funcionalidades')}>
                            <span className="accordion-title"><SvgZap /> Visibilidad Módulos</span>
                            <span className="accordion-chevron">▼</span>
                        </div>
                        {seccionAbierta === 'funcionalidades' && (
                            <div className="accordion-content">
                                <p className="settings-subtitle" style={{ marginBottom: '15px' }}>Activa secciones. Edita su contenido en la pestaña "Gestión".</p>
                                <div className="settings-form-group">
            ...
            <div className="settings-preview-canvas">
                ...
                <div className={`preview-stage ${isAnimatingPreview ? 'animating-stage' : ''}`}>
                    {/* El botón de WhatsApp ahora vive AFUERA del scroll (shop-screen) pero ADENTRO del chasis del teléfono */}
                    <div className={previewMode === 'mobile' ? 'phone-mockup-final' : 'desktop-mockup-final'}>
                                ...
                                <div className="shop-section shop-section-tracking">
                                    <div className="tracking-inner glass-effect">
                                        ...
                                        <div style={{ display: 'flex', gap: '10px' }}>
                                            <input type="text" placeholder="#12345" style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid var(--shop-border)', background: 'var(--shop-bg)', color: 'var(--shop-text)' }} disabled />
                        ...
                        {/* EL BOTÓN WHATSAPP AHORA VIVE AQUÍ: Anclado al teléfono, no al scroll */}
                        {config.whatsapp && (
                            <div className="floating-wa-btn-fixed" style={shopStyles}><SvgWhatsApp /></div>
                        )}

...
function InventoryView() {
    ...
    // Estados de Edición y Nuevo Artículo
    ...
    const actualizarStock = (id, delta) => {
        setInventario(prev => prev.map(item => item.id === id ? { ...item, cantidad: Math.max(0, item.cantidad + delta) } : item));
    };
        ...
        <div className="inventory-wrapper animate-fade-in">
            <header className="inventory-header">
                <div className="header-title">
                    <SvgBox /><h2>Control de Stock</h2>
                </div>
                <button className="btn-add-item" onClick={() => setModalNuevo(true)}>
                        ...
                        {editandoId === item.id ? (
                            ...
                            <>
                                <div className="card-header">
                                    ...
                                    <button className="btn-edit-icon" onClick={() => iniciarEdicion(item)}><SvgEdit /></button>
                                </div>
            ...
            {modalNuevo && (
                <div className="inv-modal-overlay animate-fade-in" onClick={() => setModalNuevo(false)}>
                    <div className="inv-modal-container animate-scale-in" onClick={e => e.stopPropagation()}>
                        <div className="inv-modal-header">
                            <h3>Añadir al Inventario</h3>
                            <button className="btn-close-modal" onClick={() => setModalNuevo(false)}><SvgX /></button>
                        </div>
                        <form onSubmit={crearArticulo} className="inv-modal-body">
                            <input type="text" placeholder="Nombre del Repuesto/Accesorio *" value={nuevoItem.nombre} onChange={e => setNuevoItem({ ...nuevoItem, nombre: e.target.value })} required className="inv-form-input" />
